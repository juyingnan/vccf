...
nuclei_file_name = rf'centroids_epidermis.csv'
...
nuclei_output_name = 'nuclei_epidermis.csv'
...
traces_vessel_line = generate_line(v_df_one, name=f"Distance-{cell_dict['CD31']['legend']}",
                                   color=cell_dict['CD31']['color'], visible='legendonly', legend_group="Link")
traces_skin_line = generate_line(s_df_one, name=f"Distance-{cell_dict['Skin']['legend']}",
                                 color=cell_dict['Skin']['color'], visible=True, legend_group="Link")
...
fig = make_subplots(
    ...
    subplot_titles=[f'Vascular Common Coordinate Framework 3D Visualization {main_subtitle}',
                    f'Distance to Skin Surface{hist_subtitle}', ]  # f'Distance to Endothelial Cells{hist_subtitle}',
)
...
fig.write_html(os.path.join(nuclei_root_path, f"epidermis_region_{region_index}.html"))
if show_html:
    fig.show()
